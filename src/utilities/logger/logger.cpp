...
inline const char *STATE_MAP[5] = {"NEW", "READY", "RUNNING", "BLOCKED",
                                   "EXIT"};

inline const char *PROCESS_PRIORITY_MAP[4] = {"SYSTEM", "INTERACTIVE", "NORMAL",
                                              "BATCH"};

inline const char *EVENT_MAP[8] = {"PROCESS_ARRIVED",
...
void Logger::print_state_transition(const std::shared_ptr<Event> event,
                                    ThreadState before_state,
                                    ThreadState after_state) const {
  /*
  ...
  */
  ...
  std::string message =
      fmt::format("Transitioned from {} to {}", STATE_MAP[before_state],
                  STATE_MAP[after_state]);

  print_verbose(event, event->thread, message);
}

void Logger::print_verbose(const std::shared_ptr<Event> event,
...
void Logger::print_per_thread_metrics(std::shared_ptr<Process> process) const {
  ...
  */
  ...
  message = fmt::format("Process {} [{}]:\n", process->process_id,
                        PROCESS_PRIORITY_MAP[process->priority]);
  std::cout << message;
